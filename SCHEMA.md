...
Эта база хранится в GitHub-репозитории `gihar/ai-knowledgebase` и должна синхронизироваться при каждой работе.

- **Перед началом любой работы с базой всегда делай `git pull --ff-only`** в `/home/user/ai-knowledgebase`, чтобы не работать поверх устаревшей локальной копии.
- Если `git pull --ff-only` не проходит из-за локальных изменений или расхождения истории, остановись и разберись с состоянием репозитория; не перезаписывай локальные или удалённые изменения молча.
- После любых изменений в базе: проверь `git status`, добавь изменённые файлы, сделай осмысленный commit и **обязательно выполни `git push`** в `origin/main`.
- В итоговом сообщении пользователю указывай, что изменения запушены, и добавляй короткий SHA commit.
- Не коммить временные файлы, секреты, токены, приватные ключи, cookie, `.env` и локальные кеши.
